...
import { r2Upload, r2Get, r2Delete, mediaKey, avatarKey } from '../utils/r2.js';

const MAX_IMAGE_BYTES = 5 * 1024 * 1024;  // 5MB
const MAX_VOICE_BYTES = 2 * 1024 * 1024;  // 2MB
...
async function uploadMedia(request, env, ctx, user) {
  ...
  const key = mediaKey(user.subId, type, filename);

  ctx.waitUntil(r2Upload(env, key, bytes, contentType));
...
async function uploadAvatar(request, env, ctx, user) {
  ...
  const key = avatarKey(user.subId, user.contactCode);
  ctx.waitUntil(r2Upload(env, key, bytes, file.type));
